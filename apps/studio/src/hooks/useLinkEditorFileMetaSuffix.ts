...
function resolveLinkTextForSave({
  ...
  if (!showLinkText) return linkText
  if (getLinkHrefType(linkHref) !== LINK_TYPES.File) return linkText

  const base = stripFileDownloadLinkMetaSuffix(linkText)
  return base + (fileMetaSuffix ?? "")
}

interface UseLinkEditorFileMetaSuffixParams {
  /** Raw link text from the editor when the modal opens (may include `[PDF, 1 MB]`). */
...
export function useLinkEditorFileMetaSuffix({
  ...
  const strippedLinkTextForForm = stripFileDownloadLinkMetaSuffix(
    initialLinkText ?? "",
  )
